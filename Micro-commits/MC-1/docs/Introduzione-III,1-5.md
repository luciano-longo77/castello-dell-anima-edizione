...
La trascrizione adottata in questo micro‑commit si colloca integralmente nel quadro metodologico delineato nel documento di progetto. Il **livello documentario** dell'edizione è costituito da una trascrizione diplomatico‑conservativa del manoscritto autografo: ogni fenomeno materiale, grafico e linguistico è trattato come dato filologicamente e storicamente rilevante e viene conservato integralmente. Su di esso si innesta, **in parallelo e senza sostituirlo**, un **livello editoriale** (regolarizzazione grafica, scioglimento delle abbreviazioni, correzione degli errori materiali), codificato con `<choice>` e sempre attribuito (`@resp`/`@cert`). Nessuna normalizzazione è dunque *tacita*: ogni intervento è esplicito, reversibile e verificabile a schema, e la lezione documentaria non viene mai soppressa. Una trascrizione ampiamente regolarizzata, a soli fini di leggibilità o di elaborazione computazionale, sarebbe invece un livello *derivato* che non fa parte del testo dell'edizione. 
...
Sotto l'etichetta «due livelli» convivono in realtà **quattro operazioni filologicamente distinte**, che è importante non confondere:

| Coppia / apparato | Operazione | Statuto filologico |
|---|---|---|
