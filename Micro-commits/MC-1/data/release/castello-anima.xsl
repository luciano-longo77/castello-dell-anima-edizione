...
<xsl:stylesheet version="1.0"
  ...
  <xsl:param name="page" select="'text'"/>
  ...
  <xsl:template match="/">
    <html lang="it">
      <head>
        ...
        <title><xsl:value-of select="//tei:titlePart[@type='main']"/><xsl:choose><xsl:when test="$page='header'"> — teiHeader</xsl:when><xsl:otherwise> — edizione di lettura</xsl:otherwise></xsl:choose></title>
        <style><xsl:call-template name="css"/></style>
      </head>
      <xsl:choose>

      <!-- ============ PAGINA A: teiHeader completo ============ -->
      <xsl:when test="$page='header'">
      <body class="page-hdr">
        <header class="toolbar">
          <div class="tb-title">teiHeader — <xsl:value-of select="//tei:titlePart[@type='main']"/></div>
          <div class="tb-controls">
            <a class="linkbtn" href="castello-anima-lettura.html">📖 Testo dell'edizione →</a>
          </div>
        </header>
        <main class="page hdr"><xsl:call-template name="header-page"/></main>
        <footer class="colophon">
          <p class="gen">Vista del <code>teiHeader</code> generata da <code>castello-anima.xsl</code>
             (<code>--stringparam page header</code>). La cronologia redazionale (<code>revisionDesc</code>)
             è esclusa da questa pagina. Fonte autorevole: i dati TEI in <code>data/</code>.</p>
        </footer>
      </body>
      </xsl:when>

      <!-- ============ PAGINA B: testo dell'edizione ============ -->
      <xsl:otherwise>
      <body class="mode-i show-app show-stati show-enc">
        ...
        <nav class="chapnav" aria-label="Capitoli">
          <a href="castello-anima-teiHeader.html" class="nav-sch">📋 teiHeader completo</a>
        ...
        <section class="legenda" id="legenda">
          ...
          <p class="leg-intro">Vocabolario controllato ancorato al testo con <code>term/@ref</code>;
             definizioni dal <code>catDesc</code> della tassonomia. Metadati completi nella
             <a href="castello-anima-teiHeader.html">pagina del teiHeader</a>.</p>
          <dl>
        ...
        <footer class="colophon">
          <p class="gen">Pagina di lettura generata da <code>castello-anima.xsl</code> sul master
             risolto TEI. Metadati completi: <a href="castello-anima-teiHeader.html">pagina del teiHeader</a>.
  ...
  <xsl:template name="header-page">
    <div class="hdr-doc">
      <h1 class="hdr-h1">Descrizione dell'edizione — <code>teiHeader</code></h1>
      <p class="hdr-sub">Metadati completi del micro-commit MC-1 (Libro III, capp. I–V). Esclusa la cronologia redazionale.</p>

      <section class="hsec"><h2>1 · Descrizione bibliografica</h2>
        <dl class="meta">
      ...
      <section class="hsec"><h2>2 · Note descrittive</h2>
      ...
      <section class="hsec"><h2>3 · Il manoscritto</h2>
        <dl class="meta">
      ...
      <section class="hsec"><h2>4 · Mani</h2>
        <ul class="hands"><xsl:for-each select="//tei:handNote"><li><span class="sig"><xsl:value-of select="@xml:id"/></span> (<xsl:value-of select="@medium"/>): <xsl:value-of select="normalize-space(.)"/></li></xsl:for-each></ul>
      </section>
      </xsl:if>

      <section class="hsec"><h2>5 · Stati genetici (testimoni)</h2>
        <xsl:for-each select="//tei:listWit/tei:listWit">
          <p class="wit-grp"><b><xsl:value-of select="normalize-space(tei:head)"/></b></p>
          <ul class="wit"><xsl:for-each select="tei:witness"><li><span class="sig"><xsl:value-of select="@xml:id"/></span> — <xsl:value-of select="normalize-space(.)"/></li></xsl:for-each></ul>
        </xsl:for-each>
        <xsl:if test="//tei:creation/tei:listChange">
          <p class="wit-grp"><b>Campagne scrittorie (genesi)</b></p>
          <ul class="wit"><xsl:for-each select="//tei:creation/tei:listChange/tei:change"><li><xsl:value-of select="normalize-space(.)"/></li></xsl:for-each></ul>
        </xsl:if>
      </section>

      <section class="hsec"><h2>6 · Fonti e bibliografia</h2>
      ...
      <section class="hsec"><h2>7 · Criteri di codifica</h2>
        ...
        <xsl:if test="//tei:projectDesc"><h3>Il progetto e il modello</h3><xsl:for-each select="//tei:projectDesc/tei:p"><p class="sch-p"><xsl:value-of select="normalize-space(.)"/></p></xsl:for-each></xsl:if>
        <xsl:if test="//tei:refsDecl"><h3>Riferimenti interni</h3><xsl:for-each select="//tei:refsDecl/tei:p"><p class="sch-p"><xsl:value-of select="normalize-space(.)"/></p></xsl:for-each></xsl:if>
        ...
        <xsl:if test="//tei:variantEncoding"><p class="sch-p"><b>Codifica delle varianti:</b> metodo <code><xsl:value-of select="//tei:variantEncoding/@method"/></code>, collocazione <code><xsl:value-of select="//tei:variantEncoding/@location"/></code>.</p></xsl:if>
        <xsl:if test="//tei:schemaRef"><p class="sch-p"><b>Schema:</b> <code><xsl:value-of select="//tei:schemaRef/@url"/></code> (<xsl:value-of select="//tei:schemaRef/@type"/>).</p></xsl:if>
      </section>

      <section class="hsec"><h2>8 · Tassonomie</h2>
        <h3>Stati mistici (<xsl:value-of select="count(//tei:taxonomy[@xml:id='stati-mistici']/tei:category)"/>)</h3>
      ...
      <section class="hsec"><h2>9 · Lingue, persone ed enti</h2>
        <h3>Lingue</h3>
        <ul class="langs"><xsl:for-each select="//tei:langUsage/tei:language"><li><span class="sig"><xsl:value-of select="@xml:id"/></span>: <xsl:value-of select="normalize-space(.)"/></li></xsl:for-each></ul>
        <xsl:if test="//tei:particDesc//tei:person">
          <h3>Persone</h3>
          <ul class="persons"><xsl:for-each select="//tei:particDesc//tei:person"><li><b><xsl:value-of select="normalize-space((.//tei:persName | .//tei:name)[1])"/></b><xsl:if test="tei:note"> — <xsl:value-of select="normalize-space((tei:note)[1])"/></xsl:if></li></xsl:for-each></ul>
        </xsl:if>
        <xsl:if test="//tei:particDesc//tei:org">
          <h3>Enti</h3>
          <ul class="persons"><xsl:for-each select="//tei:particDesc//tei:org"><li><b><xsl:value-of select="normalize-space((.//tei:orgName | .//tei:name)[1])"/></b><xsl:if test="tei:note"> — <xsl:value-of select="normalize-space((tei:note)[1])"/></xsl:if></li></xsl:for-each></ul>
        </xsl:if>
      </section>
    </div>
  </xsl:template>
